...
function startVoiceCapture() {
    ...
    var lang = '""" + str(st.session_state.get("language", "en")) + """';
...
def show_file_complaint():
    """Streamlit UI for filing complaints - NO BACKEND CODE HERE"""
    lang = st.session_state.get("language", "en")
    ...
    # Submit button
    st.markdown("---")
    if st.button(f"🚀 {t('submit')}", use_container_width=True, key="fc_sub"):
        if not description:
            ...
            try:
                ...
                if data.get("success"):
                    ...
                    <div class="card" style="border:2px solid #16a34a;border-radius:20px;text-align:center;padding:30px;">
                        ...
                        <div style="margin-top:8px;font-size:0.8rem;color:#d97706;">
                            ⏱️ SLA Deadline: {sla_deadline}
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    st.error(data.get("detail", data.get("error", "Error filing complaint")))
            except requests.exceptions.ConnectionError:
                st.error("⚠️ Cannot connect to backend. Make sure the server is running on port 8000")
