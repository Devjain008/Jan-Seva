...
# ─────────────────────────────────────────────────────────────────────────────
#  Stubs — delete when integrating into your main app
# ─────────────────────────────────────────────────────────────────────────────
def logout():
...
/* ══════════════════════════════════════════════════════════════
   FONTS
══════════════════════════════════════════════════════════════ */
@import url('https://fonts.googleapis.com/css2?family=Outfit:wght@500;600;700;800&family=Plus+Jakarta+Sans:wght@400;500;600;700&family=Noto+Sans+Devanagari:wght@400;500;600;700&display=swap');
...
section[data-testid="stSidebar"] {
    ...
    border-right: 1.5px solid #EAE6DF !important;
    min-width: 268px !important;
    max-width: 268px !important;
    box-shadow: 2px 0 12px rgba(28,28,26,0.06) !important;
}
...
section[data-testid="stSidebar"]::-webkit-scrollbar-thumb {
    background: #D6D1C8; border-radius: 4px;
}

/* ══════════════════════════════════════════════════════════════
   ★ THE GAP FIX ★
   All wrapper divs use display:contents — they become
   invisible to the layout engine, contributing ZERO height.
   The button inside flows as if the wrapper doesn't exist.
   Active/idle/logout states only change colors, never geometry.
══════════════════════════════════════════════════════════════ */
.sb-nav-active,
.sb-nav-idle,
.sb-logout,
...
/* ══════════════════════════════════════════════════════════════
   ...
   CRITICAL: padding, border-width, border-radius, margin are
   NEVER changed between active/idle/hover/any state.
   Only background, color, box-shadow change.
══════════════════════════════════════════════════════════════ */
section[data-testid="stSidebar"] div[data-testid="stButton"] > button {
    /* ── Geometry — LOCKED, never changes ── */
    ...
    /* ── No transforms (they cause layout jitter in Streamlit) ── */
    transform: none !important;
}

/* Hover — only bg + color, NO geometry change */
section[data-testid="stSidebar"] div[data-testid="stButton"] > button:hover {
    background: #FFF3E8 !important;
    color: #B84D00 !important;
    border-color: #FDDCB5 !important;
    box-shadow: none !important;
    transform: none !important;
}
...
/* ── ACTIVE NAV ITEM
   Left accent via inset shadow = ZERO width/padding change ── */
.sb-nav-active div[data-testid="stButton"] > button {
    background: #FFF0E2 !important;
    color: #B84D00 !important;
    border-color: #FDDCB5 !important;
    font-weight: 700 !important;
    box-shadow: inset 3px 0 0 0 #E8660A !important;
}
.sb-nav-active div[data-testid="stButton"] > button:hover {
...
══════════════════════════════════════════════════════════════ */
.sb-brand {
    background: linear-gradient(150deg, #1C1C1A 0%, #2D1800 50%, #1C1C1A 100%);
    padding: 20px 16px 16px;
    text-align: center;
    position: relative;
    overflow: hidden;
    /* No margin — flush with top */
}
.sb-brand::before {
    content: '';
    position: absolute; top: -40px; right: -30px;
    width: 120px; height: 120px;
    background: rgba(232,102,10,0.20);
    border-radius: 50%;
    pointer-events: none;
}
.sb-brand::after {
    ...
    height: 1px;
    background: linear-gradient(90deg, transparent, rgba(232,102,10,0.4), transparent);
}
.sb-brand-icon-wrap {
    width: 48px; height: 48px;
    background: rgba(232,102,10,0.18);
    border: 1.5px solid rgba(232,102,10,0.40);
    border-radius: 14px;
...
.sb-tricolor {
    ...
    background: linear-gradient(90deg,
        #FF9933 0% 33.3%, #FFFFFF 33.3% 66.6%, #138808 66.6% 100%);
    opacity: 0.55; width: 56%;
    margin: 12px auto 0;
...
.sb-profile-name {
    ...
    color: #1C1C1A; line-height: 1.2;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    max-width: 155px;
}
.sb-profile-sub {
    font-size: 10.5px; color: #7A7568; font-weight: 400;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    max-width: 155px; margin-top: 1px;
}
...
.sb-ai-icon-wrap {
    ...
    background: linear-gradient(135deg, #E8660A, #F59E0B);
    border-radius: 9px;
    display: flex; align-items: center; justify-content: center;
    font-size: 15px; flex-shrink: 0;
}
.sb-ai-title { font-size: 11.5px; font-weight: 700; color: #92400E; line-height: 1.2; }
.sb-ai-sub { font-size: 10px; color: #B45309; margin-top: 1px; }
...
.sb-section-label {
    ...
    color: #A8A199; padding: 14px 14px 5px;
    display: block;
}
.sb-divider {
    height: 1px;
    background: linear-gradient(90deg,
        transparent 0%, #EAE6DF 20%, #EAE6DF 80%, transparent 100%);
    margin: 5px 10px;
}
...
.sb-welcome {
    ...
    border: 1px solid #EAE6DF;
    border-radius: 12px;
    padding: 16px; margin: 10px 10px 0;
    text-align: center;
}
.sb-welcome-icon { font-size: 28px; margin-bottom: 8px; display: block; }
.sb-welcome-title {
    font-family: 'Outfit', sans-serif !important;
    font-size: 13px; font-weight: 700;
    color: #1C1C1A; margin-bottom: 4px;
}
.sb-welcome-sub { font-size: 11px; color: #7A7568; line-height: 1.5; }
...
.sb-footer {
    text-align: center;
    font-size: 10px; color: #A8A199;
    padding: 10px 12px 14px;
    border-top: 1px solid #EAE6DF;
    margin: 8px 10px 0;
    line-height: 1.9;
}
.sb-footer-flag { font-size: 20px; display: block; margin-bottom: 3px; }

/* ══════════════════════════════════════════════════════════════
   RESPONSIVE — collapse gracefully below 300px
══════════════════════════════════════════════════════════════ */
@media (max-width: 300px) {
    ...
    .sb-brand-name { font-size: 13px !important; }
    .sb-profile-name { max-width: 110px !important; }
}
</style>
""", unsafe_allow_html=True)
...
def _sb_gap(px: int = 6):
    st.markdown(f'<div style="height:{px}px;flex-shrink:0;"></div>', unsafe_allow_html=True)


# ─────────────────────────────────────────────────────────────────────────────
#  Nav button  ← THE CORE FIX
# ─────────────────────────────────────────────────────────────────────────────

def _nav_button(icon: str, label: str, screen: str, badge: int = 0):
    """
    Render one sidebar nav button.

    FIX SUMMARY
    ───────────
    Both the active wrapper (.sb-nav-active) and the idle wrapper
    (.sb-nav-idle) have `display: contents` in CSS, making them
    **invisible to the layout engine**.  Streamlit's own spacing divs
    around st.button() stay constant — they are never touched.
    The button's padding / border-width / margin / border-radius are
    identical in ALL states; only background + color + box-shadow change.
    Result: zero layout shift when clicking between items.
    """
    is_active = st.session_state.get("screen") == screen
    badge_txt = f"  ·  {badge}" if badge else ""
    btn_label = f"{icon}  {label}{badge_txt}"
    cls = "sb-nav-active" if is_active else "sb-nav-idle"
...
_AVATAR_BG = {
    ...
    "👑": "linear-gradient(135deg,#E8660A,#F59E0B)",
}
_BADGE_STYLE = {
    "citizen":  ("color:#4338CA;background:#EEF2FF;",  "Citizen",  "नागरिक"),
    "official": ("color:#0369A1;background:#E0F2FE;",  "Official", "अधिकारी"),
    "admin":    ("color:#C2410C;background:#FFF4EC;",  "Admin",    "एडमिन"),
}

def _sb_profile(avatar: str, name: str, sub: str, role_key: str, lang: str):
    av_bg   = _AVATAR_BG.get(avatar, _AVATAR_BG["👤"])
    bstyle, ben, bhi = _BADGE_STYLE.get(role_key, ("", "", ""))
    blabel  = bhi if lang == "hi" else ben
    st.markdown(f"""
<div class="sb-profile">
    <div class="sb-profile-avatar" style="background:{av_bg};">{avatar}</div>
    <div style="overflow:hidden;flex:1;min-width:0;">
        <div class="sb-profile-role-badge" style="{bstyle}">{blabel}</div>
        <div class="sb-profile-name">{name}</div>
        <div class="sb-profile-sub">{sub}</div>
    </div>
</div>
""", unsafe_allow_html=True)
# =============================================================================
# SIDEBAR – FIXED, NO LEAKING TEXTS, BILINGUAL, COMPACT
# =============================================================================

def render_sidebar():
    # Only ONE sidebar CSS – compact buttons, no extra spacing
    st.markdown("""
    <style>
                /* =========================================================
                REMOVE STREAMLIT DEFAULT MULTIPAGE SIDEBAR
                ========================================================= */
    ...
    /* Kill default sidebar padding and nav */
    section[data-testid="stSidebar"] > div:first-child { padding: 0 !important; }
    section[data-testid="stSidebar"] .block-container { padding: 0 !important; margin: 0 !important; }
    section[data-testid="stSidebarNav"], section[data-testid="stSidebarNavItems"] { display: none !important; }
...
.sb-top-card{
    ...
    box-shadow:
        0 10px 30px rgba(29,78,216,0.18);

    text-align: center;
}

/* background building glow */
...
.sb-login-sub{
    ...
    /* Compact buttons – same geometry for all */
    ...
    div[data-testid="stSidebar"] div[data-testid="stButton"] > button:hover {
        ...
        border-color: #FDD0A0 !important;
        color: #B84D00 !important;
    }
    /* Active style */
    .sb-active div[data-testid="stButton"] > button {
        background: #FFF0E2 !important;
        border-left: 3px solid #E8660A !important;
        font-weight: 700 !important;
    ...
    /* Divider */
    .sb-divider { height: 1px; background: #E8E4DC; margin: 6px 12px; }
    /* Section label */
    .sb-section { font-size: 8.5px; font-weight: 700; letter-spacing: 1px; color: #B0A898; padding: 10px 13px 3px; }
    /* Footer */
    .sb-footer { border-top: 1px solid #E8E4DC; margin: 12px 12px 0; padding: 10px 0 12px; text-align: center; font-size: 9px; color: #A8A199; }
    .sb-footer-flag { font-size: 16px; display: block; margin-bottom: 4px; }
    </style>
    """, unsafe_allow_html=True)
    ...
    hi = (lang == "hi")
    def _(en, hi_text=""):
        return hi_text if (hi and hi_text) else en

    role = st.session_state.get("role")  # None, "user", "official", "admin"

    with st.sidebar:
        # 1. Brand (never changes)
        st.markdown("""
        <div class="sb-brand">
            <div class="sb-brand-name">Jan Seva Portal</div>
            <div class="sb-brand-sub">जन सेवा पोर्टल</div>
            <div class="sb-tricolor"></div>
        </div>
        """, unsafe_allow_html=True)

        # 2. Simple welcome / profile info (no AI badge to avoid complexity)
        if role == "user":
            user = st.session_state.get("user") or {}
            name = user.get("name", _("Citizen", "नागरिक"))
            phone = user.get("phone", "")
            st.sidebar.markdown(f"**👤 {name}**  \n📞 {phone}")
        elif role == "official":
            off = st.session_state.get("official") or {}
            name = off.get("name", _("Official", "अधिकारी"))
            dept = off.get("department", "")
            st.sidebar.markdown(f"**🏢 {name}**  \n{dept}")
        elif role == "admin":
            st.sidebar.markdown("**👑 " + _("System Admin", "सिस्टम एडमिन") + "**")
        else:
            st.markdown("""
<div class="sb-top-wrap">

    <div class="sb-top-card">
    ...
    <div class="sb-login-card">
        ...
        <div>
            <div class="sb-login-title">
                Jan Seva Portal
            </div>

            <div class="sb-login-sub">
                Login to continue
            </div>
        </div>
...
""", unsafe_allow_html=True)
        ...
        def nav_button(icon, label, screen, badge=0):
            active = (st.session_state.get("screen") == screen)
            btn_label = f"{icon}  {label}" + (f"  ({badge})" if badge else "")
            if active:
                st.markdown('<div class="sb-active">', unsafe_allow_html=True)
            if st.sidebar.button(btn_label, key=f"nav_{screen}", use_container_width=True):
                st.session_state.screen = screen
                st.rerun()
            if active:
                st.markdown('</div>', unsafe_allow_html=True)
        ...
        if role == "user":
            st.markdown('<div class="sb-section">' + _("MAIN MENU", "मुख्य मेनू") + '</div>', unsafe_allow_html=True)
            nav_button("🏠", _("Dashboard", "डैशबोर्ड"), "user_dashboard")
            nav_button("📢", _("File Complaint", "शिकायत दर्ज"), "file_complaint")
            nav_button("🔍", _("Track Complaint", "शिकायत ट्रैक"), "tracking")
            nav_button("📜", _("Govt Schemes", "सरकारी योजनाएं"), "schemes")
            nav_button("🤖", _("AI Assistant", "AI सहायक"), "assistant")
            # Notification badge (dummy – replace with real count)
            unread = 0
            nav_button("🔔", _("Notifications", "सूचनाएं"), "notifications", unread)

        elif role == "official":
            st.markdown('<div class="sb-section">' + _("WORKSPACE", "कार्यक्षेत्र") + '</div>', unsafe_allow_html=True)
            nav_button("📊", _("Dashboard", "डैशबोर्ड"), "official_dashboard")
            nav_button("📋", _("Complaints", "शिकायतें"), "official_complaints")
            nav_button("🏆", _("Leaderboard", "लीडरबोर्ड"), "official_leaderboard")

        elif role == "admin":
            st.markdown('<div class="sb-section">' + _("MAIN MENU", "मुख्य मेनू") + '</div>', unsafe_allow_html=True)
            nav_button("📊", _("Dashboard", "डैशबोर्ड"), "admin_dashboard")
            nav_button("🏢", _("Departments", "विभाग"), "admin_departments")
            nav_button("👥", _("Officials", "अधिकारी"), "admin_officials")
            nav_button("📢", _("All Complaints", "सभी शिकायतें"), "admin_complaints")
            nav_button("🏆", _("Leaderboard", "लीडरबोर्ड"), "admin_leaderboard")
            nav_button("🔮", _("Predictive AI", "पूर्वानुमान AI"), "predictive_analytics")
            nav_button("⏱️", _("SLA Tracking", "SLA ट्रैकिंग"), "sla_management")
            nav_button("🏙️", _("City Health", "शहर स्वास्थ्य"), "city_health_score")
            nav_button("📈", _("Live Dashboard", "लाइव डैशबोर्ड"), "live_dashboard")
            nav_button("🗺️", _("Heatmap", "हीटमैप"), "admin_heatmap")
            nav_button("📜", _("Schemes", "योजनाएं"), "admin_schemes")

        else:  # Not logged in
            st.markdown('<div class="sb-section">' + _("LOGIN", "लॉगिन") + '</div>', unsafe_allow_html=True)
            nav_button("👤", _("Citizen", "नागरिक"), "user_login")
            nav_button("🏢", _("Official", "अधिकारी"), "official_login")
            nav_button("👑", _("Admin", "एडमिन"), "admin_login")

        # 4. Preferences
        st.markdown('<div class="sb-divider"></div>', unsafe_allow_html=True)
        st.markdown('<div class="sb-section">' + _("PREFERENCES", "प्राथमिकताएं") + '</div>', unsafe_allow_html=True)
        nav_button("🌐", _("Switch to Hindi", "Switch to English"), "lang_toggle")
        nav_button("❓", _("Help & Support", "सहायता"), "help")

        # 5. Logout
        if role:
            st.markdown('<div class="sb-divider"></div>', unsafe_allow_html=True)
            st.markdown('<div class="sb-logout">', unsafe_allow_html=True)
            if st.sidebar.button(f"🔓  {_('Logout', 'लॉगआउट')}", key="logout_button", use_container_width=True):
                # Clear session
                for k in list(st.session_state.keys()):
                    del st.session_state[k]
                st.rerun()
        ...
        st.markdown(f"""
        <div class="sb-footer">
            <span class="sb-footer-flag">🇮🇳</span>
            <strong>{_("Digital India Initiative", "डिजिटल इंडिया")}</strong><br>
            {_("Powering Smart Governance", "स्मार्ट शासन की शक्ति")}
        </div>
        """, unsafe_allow_html=True)


# =============================================================================
# LANGUAGE TOGGLE HANDLER – call AFTER render_sidebar() in your main flow
# =============================================================================
def handle_special_screens():
    if st.session_state.get("screen") == "lang_toggle":
        new_lang = "hi" if st.session_state.get("language", "en") == "en" else "en"
        st.session_state.language = new_lang
        # Return to previous screen if stored, else a safe default
        prev = st.session_state.get("prev_screen", "login_type")
        st.session_state.screen = prev
        st.rerun()
# ═════════════════════════════════════════════════════════════════════════════
# ROUTER
# ═════════════════════════════════════════════════════════════════════════════
...
_CONTENT = {
    "en": {
        # Hero
        "portal_name_en":   "Jan Seva Portal",
        "portal_name_hi":   "जन सेवा पोर्टल",
        "hero_sub":         "AI-Powered Citizen Grievance Management System",
        "badge_digital":    "🇮🇳 Digital India",
        "badge_gov":        "⚙️ Smart Governance",
        "badge_secure":     "🔒 Secure & Trusted",
        "badge_mobile":     "📱 Mobile Ready",

        # Stats
        "stat1_num":  "2.4<span class='jsp-stat-unit'>M+</span>",
        "stat1_lbl":  "Complaints Resolved",
        "stat2_num":  "98<span class='jsp-stat-unit'>%</span>",
        "stat2_lbl":  "Resolution Rate",
        "stat3_num":  "4.8<span class='jsp-stat-unit'>★</span>",
        "stat3_lbl":  "Citizen Satisfaction",
        "stat4_num":  "72<span class='jsp-stat-unit'>hr</span>",
        "stat4_lbl":  "Avg Resolution Time",

        # Language section
        "lang_sec_title":   "🌐 Choose Your Language",
        "lang_sec_sub":     "अपनी भाषा चुनें · Select your preferred language to continue",
        "card1_flag":       "🇬🇧",
        "card1_name":       "English",
        "card1_native":     "International Language",
        "card1_pill":       "Widely Spoken",
        "card2_flag":       "🇮🇳",
        "card2_name":       "हिंदी",
        "card2_native":     "भारत की राष्ट्रीय भाषा",
        "card2_pill":       "National Language",
        "btn_en":           "🌐 Continue in English",
        "btn_hi":           "🇮🇳 हिंदी में जारी रखें",

        # Transparency
        "portal_title":     "📊 Public Transparency Portal",
        "portal_sub":       "View real-time grievance statistics, department performance & open data reports",
        "portal_btn":       "📊 Open Transparency Portal",

        # How it works
        "how_sec_title":    "📚 How It Works",
        "how_sec_sub":      "Simple 3-step process to get your grievance resolved",
        "step1_num":        "Step 01",
        "step1_icon":       "📝",
        "step1_title":      "File a Complaint",
        "step1_desc":       "Describe your issue in text or voice, add your location and photos. Our AI auto-categorises it and routes it to the right department instantly.",
        "step2_num":        "Step 02",
        "step2_icon":       "⚙️",
        "step2_title":      "Track Progress",
        "step2_desc":       "Receive real-time SMS and email alerts. Watch live status updates on a transparent dashboard and escalate automatically if SLA is breached.",
        "step3_num":        "Step 03",
        "step3_icon":       "✅",
        "step3_title":      "Get Resolution",
        "step3_desc":       "Officials submit action reports with photos. Rate your experience and give feedback to improve government service quality continuously.",

        # Schemes
        "schemes_sec_title": "🏅 Government Schemes",
        "schemes_sec_sub":   "Major citizen welfare initiatives — file complaints or queries about these schemes",
        "s1_name":  "PM Awas Yojana",
        "s1_desc":  "Housing for All — report issues with allotment, construction quality or beneficiary enrolment.",
        "s2_name":  "Jal Jeevan Mission",
        "s2_desc":  "Tap water for every household — complaints about supply disruptions or pipeline damage.",
        "s3_name":  "PM KUSUM / SAUBHAGYA",
        "s3_desc":  "Solar energy & rural electrification — report outages, metering issues or connection delays.",
        "s4_name":  "PM Gram Sadak Yojana",
        "s4_desc":  "All-weather rural roads — raise concerns about poor quality, potholes or missing links.",
        "s5_name":  "PM Kisan Samman Nidhi",
        "s5_desc":  "Direct income support to farmers — flag payment delays, KYC issues or enrolment problems.",
        "s6_name":  "Ayushman Bharat",
        "s6_desc":  "Health coverage up to ₹5 lakh — report denial of treatment, card issues or hospital fraud.",

        # Features
        "feat_sec_title": "✨ Platform Features",
        "feat1_title": "AI-Powered",       "feat1_sub": "Smart auto-categorisation",
        "feat2_title": "Voice Input",      "feat2_sub": "Speak your complaint",
        "feat3_title": "GPS Location",     "feat3_sub": "Auto-detect address",
        "feat4_title": "Real-time Alerts", "feat4_sub": "SMS & email notifications",
        "feat5_title": "SLA Tracking",     "feat5_sub": "Time-bound resolution",
        "feat6_title": "Analytics",        "feat6_sub": "Data-driven insights",
        "feat7_title": "Secure & Private", "feat7_sub": "End-to-end encryption",

        # Help
        "help_title": "📞 Need Help? We're Here.",
        "help_email": "support@janseva.gov.in",
        "help_hours": "Working Hours: Monday – Saturday, 9:00 AM to 6:00 PM IST",
        "help_num":   "📲 1800-XXX-XXXX",
        "help_num_sub": "Toll-Free Helpline",

        # Footer
        "footer_copy":  "© 2024 Jan Seva Portal",
        "footer_sub1":  "An Initiative by the Government of India &nbsp;|&nbsp; Version 2.0",
        "footer_sub2":  "Empowering Citizens &nbsp;·&nbsp; Ensuring Accountability &nbsp;·&nbsp; Enhancing Governance",
        "footer_sub3":  "Built on <strong>Digital India</strong> infrastructure &nbsp;·&nbsp; Data localised in India",
        "footer_priv":  "Privacy Policy",
        "footer_terms": "Terms of Use",

        # Voice greeting
        "voice_msg":  "Welcome to Jan Seva Portal. An AI-powered Citizen Grievance Management System. Please select your preferred language to continue.",
        "voice_lang": "en-IN",

        # Lang toggle button
        "toggle_btn": "🇮🇳 हिंदी में देखें",
    },
    ...
    "hi": {
        ...
        "portal_name_en":   "Jan Seva Portal",
        "portal_name_hi":   "जन सेवा पोर्टल",
        "hero_sub":         "AI-संचालित नागरिक शिकायत प्रबंधन प्रणाली",
        "badge_digital":    "🇮🇳 डिजिटल इंडिया",
        "badge_gov":        "⚙️ स्मार्ट शासन",
        "badge_secure":     "🔒 सुरक्षित एवं विश्वसनीय",
        "badge_mobile":     "📱 मोबाइल तैयार",

        # Stats
        "stat1_num":  "2.4<span class='jsp-stat-unit'>M+</span>",
        "stat1_lbl":  "शिकायतें हल हुईं",
        "stat2_num":  "98<span class='jsp-stat-unit'>%</span>",
        "stat2_lbl":  "समाधान दर",
        "stat3_num":  "4.8<span class='jsp-stat-unit'>★</span>",
        "stat3_lbl":  "नागरिक संतुष्टि",
        "stat4_num":  "72<span class='jsp-stat-unit'>घं.</span>",
        "stat4_lbl":  "औसत समाधान समय",

        # Language section
        "lang_sec_title":   "🌐 अपनी भाषा चुनें",
        "lang_sec_sub":     "Choose your language · जारी रखने के लिए अपनी भाषा चुनें",
        "card1_flag":       "🇬🇧",
        "card1_name":       "English",
        "card1_native":     "अंतर्राष्ट्रीय भाषा",
        "card1_pill":       "व्यापक रूप से बोली जाती है",
        "card2_flag":       "🇮🇳",
        "card2_name":       "हिंदी",
        "card2_native":     "भारत की राष्ट्रीय भाषा",
        "card2_pill":       "राष्ट्रीय भाषा",
        "btn_en":           "🌐 Continue in English",
        "btn_hi":           "🇮🇳 हिंदी में जारी रखें",

        # Transparency
        "portal_title":     "📊 सार्वजनिक पारदर्शिता पोर्टल",
        "portal_sub":       "वास्तविक समय की शिकायत सांख्यिकी, विभाग प्रदर्शन और खुले डेटा रिपोर्ट देखें",
        "portal_btn":       "📊 पारदर्शिता पोर्टल खोलें",

        # How it works
        "how_sec_title":    "📚 यह कैसे काम करता है",
        "how_sec_sub":      "आपकी शिकायत हल करने की सरल 3-चरण प्रक्रिया",
        "step1_num":        "चरण 01",
        "step1_icon":       "📝",
        "step1_title":      "शिकायत दर्ज करें",
        "step1_desc":       "अपनी समस्या टेक्स्ट या आवाज़ में बताएं, स्थान और फ़ोटो जोड़ें। हमारा AI इसे स्वतः वर्गीकृत करके तुरंत सही विभाग को भेज देता है।",
        "step2_num":        "चरण 02",
        "step2_icon":       "⚙️",
        "step2_title":      "प्रगति ट्रैक करें",
        "step2_desc":       "वास्तविक समय SMS और ईमेल अलर्ट प्राप्त करें। पारदर्शी डैशबोर्ड पर लाइव स्थिति अपडेट देखें और SLA उल्लंघन पर स्वचालित एस्केलेशन करें।",
        "step3_num":        "चरण 03",
        "step3_icon":       "✅",
        "step3_title":      "समाधान पाएं",
        "step3_desc":       "अधिकारी फ़ोटो सहित कार्यवाही रिपोर्ट प्रस्तुत करते हैं। अपना अनुभव रेट करें और सरकारी सेवा की गुणवत्ता लगातार बेहतर करने में मदद करें।",

        # Schemes
        "schemes_sec_title": "🏅 सरकारी योजनाएँ",
        "schemes_sec_sub":   "प्रमुख नागरिक कल्याण पहलें — इन योजनाओं से संबंधित शिकायतें या प्रश्न दर्ज करें",
        "s1_name":  "पीएम आवास योजना",
        "s1_desc":  "सबके लिए आवास — आवंटन, निर्माण गुणवत्ता या लाभार्थी नामांकन की समस्याएं रिपोर्ट करें।",
        "s2_name":  "जल जीवन मिशन",
        "s2_desc":  "हर घर नल जल — आपूर्ति बाधा या पाइपलाइन क्षति की शिकायत करें।",
        "s3_name":  "पीएम कुसुम / सौभाग्य",
        "s3_desc":  "सौर ऊर्जा और ग्रामीण विद्युतीकरण — बिजली कटौती, मीटरिंग समस्या या कनेक्शन विलंब रिपोर्ट करें।",
        "s4_name":  "पीएम ग्राम सड़क योजना",
        "s4_desc":  "सर्वमौसम ग्रामीण सड़कें — खराब गुणवत्ता, गड्ढे या मिसिंग लिंक की समस्या उठाएं।",
        "s5_name":  "पीएम किसान सम्मान निधि",
        "s5_desc":  "किसानों को प्रत्यक्ष आय सहायता — भुगतान विलंब, KYC समस्या या नामांकन की समस्या बताएं।",
        "s6_name":  "आयुष्मान भारत",
        "s6_desc":  "₹5 लाख तक स्वास्थ्य कवरेज — उपचार अस्वीकृति, कार्ड समस्या या अस्पताल धोखाधड़ी रिपोर्ट करें।",

        # Features
        "feat_sec_title": "✨ प्लेटफ़ॉर्म की विशेषताएँ",
        "feat1_title": "AI-संचालित",       "feat1_sub": "स्मार्ट स्वतः वर्गीकरण",
        "feat2_title": "वॉयस इनपुट",       "feat2_sub": "अपनी शिकायत बोलें",
        "feat3_title": "GPS स्थान",         "feat3_sub": "पता स्वतः पहचानें",
        "feat4_title": "रियल-टाइम अलर्ट",  "feat4_sub": "SMS और ईमेल सूचनाएं",
        "feat5_title": "SLA ट्रैकिंग",     "feat5_sub": "समयबद्ध समाधान",
        "feat6_title": "विश्लेषण",          "feat6_sub": "डेटा-आधारित जानकारी",
        "feat7_title": "सुरक्षित एवं निजी", "feat7_sub": "एंड-टू-एंड एन्क्रिप्शन",

        # Help
        "help_title": "📞 सहायता चाहिए? हम यहाँ हैं।",
        "help_email": "support@janseva.gov.in",
        "help_hours": "कार्य समय: सोमवार – शनिवार, सुबह 9:00 बजे से शाम 6:00 बजे IST",
        "help_num":   "📲 1800-XXX-XXXX",
        "help_num_sub": "टोल-फ्री हेल्पलाइन",

        # Footer
        "footer_copy":  "© 2024 जन सेवा पोर्टल",
        "footer_sub1":  "भारत सरकार की एक पहल &nbsp;|&nbsp; संस्करण 2.0",
        "footer_sub2":  "नागरिकों को सशक्त बनाना &nbsp;·&nbsp; जवाबदेही सुनिश्चित करना &nbsp;·&nbsp; शासन को बेहतर बनाना",
        "footer_sub3":  "<strong>डिजिटल इंडिया</strong> अवसंरचना पर निर्मित &nbsp;·&nbsp; डेटा भारत में स्थानीयकृत",
        "footer_priv":  "गोपनीयता नीति",
        "footer_terms": "उपयोग की शर्तें",

        # Voice greeting
        "voice_msg":  "जन सेवा पोर्टल में आपका स्वागत है। एक AI-संचालित नागरिक शिकायत प्रबंधन प्रणाली। जारी रखने के लिए कृपया अपनी पसंदीदा भाषा चुनें।",
        "voice_lang": "hi-IN",

        # Lang toggle button
        "toggle_btn": "🌐 View in English",
    },
}
...
def _c(key: str) -> str:
    ...
    preview_lang = st.session_state.get("jsp_preview_lang", "en")
    return _CONTENT.get(preview_lang, _CONTENT["en"]).get(key, key)
...
<div style="background:#fff;border:1.5px solid #E5E7EB;border-radius:16px;
    ...
    display:flex;flex-direction:column;align-items:center;min-height:180px;">
    ...
    <div style="font-size:12px;color:#6B7280;line-height:1.55;flex:1;
...
def pg_login_type():
    ...
    /* Force equal-height columns so buttons align */
    ...
    # Row 2: Buttons aligned on same line
        ...
        if step == 1:
            ...
            if continue_btn:
                ...
                if errs:
                    ...
                    # All validation passed — save data and move to OTP step
...
def _dashboard_render_hero(user: dict, comps: list, lang: str) -> None:
    ...
    greet, emoji = next(
        ((_tr(lang, g, h), e) for lim, g, h, e in greet_map if hour < lim),
        (_tr(lang,"Good Evening","शुभ संध्या"), "🌙")
    )
    ...
    st.markdown(
        ...
        f'<div class="prem-hero-sub">{_tr(lang,"Welcome to your complaint dashboard","आपकी सेवा में तत्पर हैं")}</div>'
        f'<div class="prem-hero-stats">'
        f'<div class="prem-hstat h-blue"><div class="prem-hstat-num">{total}</div>'
        f'<div class="prem-hstat-lbl">📋 {_tr(lang,"Total","कुल")}</div></div>'
        f'<div class="prem-hstat h-amber"><div class="prem-hstat-num">{active}</div>'
        f'<div class="prem-hstat-lbl">🔄 {_tr(lang,"Active","सक्रिय")}</div></div>'
        f'<div class="prem-hstat h-green"><div class="prem-hstat-num">{resolved}</div>'
        f'<div class="prem-hstat-lbl">✅ {_tr(lang,"Resolved","समाधान")}</div></div>'
        f'<div class="prem-hstat h-red"><div class="prem-hstat-num">{pending}</div>'
        f'<div class="prem-hstat-lbl">⏳ {_tr(lang,"Pending","लंबित")}</div></div>'
        f'</div></div>',
        unsafe_allow_html=True,
    )
...
/* Expander — light orange (old + new Streamlit) */
.streamlit-expanderHeader{background:#FFF7ED!important;
    border:1.5px solid #FDEAD7!important;border-radius:14px!important;
    color:#0F172A!important;font-weight:700!important;font-size:.875rem!important;
    padding:12px 16px!important;transition:border-color .18s,box-shadow .18s!important;}
.streamlit-expanderHeader:hover{border-color:#F97316!important;
    box-shadow:0 0 0 3px rgba(249,115,22,0.10)!important;color:#EA580C!important;}
.streamlit-expanderContent{background:#FFFBF5!important;
    border:1.5px solid #FDEAD7!important;border-top:none!important;
    border-radius:0 0 14px 14px!important;padding:14px 16px!important;}
/* New Streamlit details/summary */
.main details{background:#FFFFFF!important;border:1.5px solid #FDEAD7!important;
    border-radius:14px!important;margin-bottom:8px!important;overflow:hidden!important;}
.main details[open]{border-color:#F97316!important;
    box-shadow:0 4px 16px rgba(249,115,22,0.10)!important;}
.main details summary{background:#FFF7ED!important;color:#0F172A!important;
    font-weight:700!important;font-size:.875rem!important;padding:12px 16px!important;
    border-radius:14px!important;border:none!important;cursor:pointer!important;}
.main details[open] summary{border-radius:14px 14px 0 0!important;
    background:#FFF1E0!important;border-bottom:1px solid #FDEAD7!important;}
.main details summary:hover{background:#FFEDD5!important;color:#EA580C!important;}
.main details > div{background:#FFFBF5!important;padding:14px 16px!important;}
.main details p,.main details span,.main details div{color:#0F172A!important;}
...
def _render_quick_actions(lang):
    st.markdown(
        f"<div class='d-section-header'>⚡ {_tr(lang,'Quick Actions','त्वरित कार्य')}</div>",
        unsafe_allow_html=True,
    )
    actions = [
        ("📢",_tr(lang,"File Complaint","शिकायत दर्ज"),"file_complaint","#E8590C","rgba(232,89,12,0.08)"),
        ("🔍",_tr(lang,"Track Status","ट्रैक करें"),   "tracking",      "#D4500B","rgba(212,80,11,0.08)"),
        ("📜",_tr(lang,"Govt Schemes","सरकारी योजनाएं"),"schemes",      "#059669","rgba(5,150,105,0.08)"),
        ("🤖",_tr(lang,"AI Assistant","एआई सहायक"),    "assistant",     "#0EA5E9","rgba(14,165,233,0.08)"),
    ]
    cols = st.columns(4)
...
def _render_notification_banner(notifs, lang):
    ...
    st.markdown(
        ...
        f"<span class='ni-badge'>{len(unread)} {_tr(lang,'new','नई')}</span></div>",
        unsafe_allow_html=True,
    )
...
def _render_feedback_section(comps, lang):
    ...
    if not need:
        return
    st.markdown(
        f"<div class='d-section-header'>✅ {_tr(lang,'Confirm Resolution','समाधान की पुष्टि करें')}</div>",
        unsafe_allow_html=True,
    )
    for c in need:
        cid      = c.get("complaint_id")
        deadline = c.get("feedback_deadline", _tr(lang,"within 2 days","2 दिनों के भीतर"))
        desc     = _clean(c.get("description",""))
        st.markdown(
            f"<div class='fb-card'><div class='fb-head'><div class='fb-icon'>✅</div>"
            f"<div><div class='fb-title'>{_tr(lang,'Complaint Resolved — Please Confirm!','शिकायत समाधान हुई — पुष्टि करें!')}</div>"
            f"<div class='fb-sub'>#{cid}</div></div></div>"
            f"<div class='fb-desc'>{_html.escape(desc[:120])}…"
            f"<br><small>⏰ {_tr(lang,'Respond by','उत्तर दें:')} {_html.escape(str(deadline))}</small>"
            f"</div></div>",
            unsafe_allow_html=True,
        )
        c1,c2 = st.columns(2)
        with c1:
            if st.button(_tr(lang,"👍 Yes, Satisfied!","👍 हाँ, संतुष्ट हूँ!"), key=f"fb_ok_{cid}"):
        ...
        with c2:
            if st.button(_tr(lang,"❌ Not Resolved — Reopen","❌ समाधान नहीं — पुनः खोलें"), key=f"fb_no_{cid}"):
...
def _render_rating_section(comps, lang):
    ...
    STAR_LABELS = {1:_tr(lang,"Very Poor","बहुत खराब"),2:_tr(lang,"Poor","खराब"),
                   3:_tr(lang,"Average","औसत"),4:_tr(lang,"Good","अच्छा"),5:_tr(lang,"Excellent","उत्कृष्ट")}
    st.markdown(
        f"<div class='d-section-header'>⭐ {_tr(lang,'Rate the Service','सेवा को रेट करें')}</div>",
        unsafe_allow_html=True,
    )
    ...
    for rt in need:
        ...
        off_name = rt.get("official_name") or officials_map.get(off_id) or _tr(lang,"the department","विभाग")
        ...
        st.markdown(
            ...
            f"<div><div class='rt-title'>{_tr(lang,'Rate the Service','सेवा को रेट करें')}</div>"
            f"<div class='rt-sub'>{_tr(lang,'Resolved by','समाधानकर्ता')}: "
            ...
            f"<div class='rt-lbl'>{_tr(lang,'Select your rating below','नीचे रेटिंग चुनें')}</div>"
            f"</div></div>",
            unsafe_allow_html=True,
        )
        scols = st.columns(5)
        ...
        comment = st.text_area(_tr(lang,"Additional feedback (optional)","अतिरिक्त प्रतिक्रिया (वैकल्पिक)"),
                                key=cmnt_key, height=80)
        if cur == 0:
            st.info(_tr(lang,"⭐ Select a star rating above before submitting.","⭐ सबमिट से पहले स्टार रेटिंग चुनें।"))
        else:
            if st.button(f"{'⭐'*cur} {_tr(lang,'Submit Rating','रेटिंग सबमिट करें')}", key=f"rate_sub_{cid}"):
                ...
                if resp.get("success"):
                    ...
                    st.success(_tr(lang,"✅ Rating submitted! Thank you.","✅ रेटिंग सबमिट हो गई! धन्यवाद।"))
...
def _render_single_complaint_card(c, lang, skip_ids, idx):
    ...
    # number label
    st.markdown(f"<div class='cc-number'>{_tr(lang,'Complaint','शिकायत')} #{idx}</div>",
                unsafe_allow_html=True)
    # open card (light border)
    st.markdown(f"<div class='cc-outer {card_cls}'>", unsafe_allow_html=True)
 
    with st.expander(exp_title, expanded=False):
        if st.button(f"🔊 {_tr(lang,'Listen','सुनें')}", key=f"voice_{cid}"):
            try:
        ...
        if img_path:
            st.markdown(
                ...
                + _tr(lang,"Complaint Evidence","शिकायत साक्ष्य") +
                "</div></div>",
                unsafe_allow_html=True,
            )
            if st.button(
                f"👁️ {_tr(lang,'View Uploaded Photo','अपलोड की गई फ़ोटो देखें')}",
                key=f"show_img_{cid}",
                use_container_width=True,
            ):
                ...
                st.image(img_url,
                         caption=_tr(lang,"Uploaded Complaint Evidence","शिकायत का प्रमाण"),
                         use_column_width=True)
 
        if sla:
            if overdue:
                st.error(f"⏰ {_tr(lang,'OVERDUE! Expected by:','अतिदेय! समय सीमा:')} {sla}")
            else:
                st.info(f"⏱️ {_tr(lang,'Expected by:','अपेक्षित:')} {sla}")
...
def _render_complaint_list(comps, lang, skip_ids):
    if not comps:
        st.markdown(
            ...
            f"{_tr(lang,'No complaints filed yet.','अभी तक कोई शिकायत दर्ज नहीं।')}"
            f"</div><div class='prem-empty-sub'>"
            f"{_tr(lang,'Use Quick Actions above to file your first complaint.','ऊपर Quick Actions से पहली शिकायत दर्ज करें।')}"
            f"</div></div>",
            unsafe_allow_html=True,
        )
        return
    ...
    st.markdown(
        f"<div class='d-section-header'>📋 {_tr(lang,'Your Complaints','आपकी शिकायतें')}</div>",
        unsafe_allow_html=True,
    )
 
    filters = [
        ("all",        _tr(lang,"All","सभी"),               "📋"),
        ("pending",    _tr(lang,"Pending","लंबित"),          "⏳"),
        ("in_progress",_tr(lang,"In Progress","प्रगति में"), "🔄"),
        ("resolved",   _tr(lang,"Resolved","समाधान"),        "✅"),
        ("closed",     _tr(lang,"Closed","बंद"),             "🔒"),
    ]
    ...
    st.markdown(
        ...
        f"{_tr(lang,'Showing','दिखा रहे हैं')} "
        f"<strong>{len(page_slice)}</strong> / <strong>{len(filtered)}</strong> "
        f"{_tr(lang,'complaints','शिकायतें')}</span></div>",
        unsafe_allow_html=True,
    )
    ...
    if shown == 0:
        st.markdown(
            ...
            f"<span class='prem-empty-icon'>🔍</span>"
            f"<div class='prem-empty-title'>"
            f"{_tr(lang,'No complaints match this filter.','इस फ़िल्टर से कोई शिकायत नहीं मिली।')}"
            f"</div></div>",
            unsafe_allow_html=True,
        )
    ...
    if total_pages > 1:
        ...
        with nav[0]:
            if page > 0 and st.button(f"← {_tr(lang,'Prev','पिछला')}", key="page_prev", use_container_width=True):
        ...
        with nav[1]:
            st.markdown(
                ...
                f"{_tr(lang,'Page','पृष्ठ')} {page+1} / {total_pages}</div>",
                unsafe_allow_html=True,
            )
        with nav[2]:
            if page < total_pages-1 and st.button(f"{_tr(lang,'Next','अगला')} →", key="page_next", use_container_width=True):
                st.session_state["dash_page"] = page+1; st.rerun()
...
def _render_notifications_footer(notifs, lang):
    ...
    if st.button(_tr(lang,"View All Notifications →","सभी सूचनाएं देखें →"), key="va_notifs_dash"):
...
def pg_user_dashboard():
    ...
    st.markdown(
        ...
        f"<div class='tip-text'><strong>{_tr(lang,'Pro Tip:','सुझाव:')}</strong> "
        f"{_tr(lang,'Track your complaint in real-time and get instant notifications on every update.','अपनी शिकायत को रियल-टाइम में ट्रैक करें और हर अपडेट पर तुरंत सूचना पाएं।')}"
        f"</div></div>",
        unsafe_allow_html=True,
    )
    ...
    with c2:
        if st.button(_tr(lang,"🔄 Refresh Dashboard","🔄 डैशबोर्ड रिफ्रेश करें"),
...
def pg_tracking():
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  CSS  — clean, modern, light-theme
    # ══════════════════════════════════════════════════════════════════════
    st.markdown("""
<style>
/* ── page background ── */
.trk-page { background: #F7F8FA; }
...
.trk-search-wrap .stTextInput > div > div > input::placeholder {
    color: #9CA3AF !important;
}

/* ── selectbox dropdown ── */
.trk-filter .stSelectbox > div > div {
    background    : #FFFFFF !important;
    border        : 1.5px solid #D1D5DB !important;
    border-radius : 10px !important;
    color         : #111827 !important;
...
.trk-filter .stSelectbox > div > div:hover {
    border-color: #9CA3AF !important;
}
.trk-filter .stSelectbox > div > div:focus-within {
    border-color : #6366F1 !important;
    box-shadow   : 0 0 0 3px rgba(99,102,241,.12) !important;
}
.trk-filter .stSelectbox [data-baseweb="select"] span,
.trk-filter .stSelectbox [data-baseweb="select"] div,
.trk-filter .stSelectbox [data-baseweb="select"] p {
    color: #111827 !important;
    font-size: 0.875rem !important;
}
.trk-filter label {
...
@keyframes tl-pulse {
    0%  { box-shadow: 0 0 0 0   rgba(139,92,246,.50); }
...
.tl-line.done { background: linear-gradient(to bottom,#6366F1,#E5E7EB); }
.tl-line.idle { background: #E5E7EB; }
.tl-info { padding: 3px 0 18px; }
.tl-lbl  { font-size: 0.80rem; font-weight: 600; color: #374151; }
.tl-time { font-size: 0.68rem; color: #9CA3AF; margin-top: 2px; }
...
.trk-empty-icon  { font-size: 3rem; opacity: .5; display: block; margin-bottom: 12px; }
.trk-empty-title { font-size: .95rem; font-weight: 700; color: #111827; margin-bottom: 6px; }
.trk-empty-sub   { font-size: .80rem; color: #6B7280; line-height: 1.6; }
...
@media (max-width: 640px) {
    ...
    .trk-card    { padding: 14px 14px; border-radius: 12px; }
    .trk-card-top{ gap: 5px; }
}
@media (max-width: 480px) {
    .trk-title   { font-size: 0.875rem; }
    .trk-desc    { font-size: 0.800rem; }
}
</style>
""", unsafe_allow_html=True)
    ...
    # ══════════════════════════════════════════════════════════════════════
    STATUS_CFG = {
        "pending":     (t("⏳ Pending",     "⏳ लंबित"),      "#FFFBEB", "#B45309", "#F59E0B"),
        "in_progress": (t("🔄 In Progress", "🔄 प्रगति"),     "#EFF6FF", "#1D4ED8", "#3B82F6"),
        "resolved":    (t("✅ Resolved",    "✅ समाधान"),     "#F0FDF4", "#15803D", "#22C55E"),
        "closed":      (t("🔒 Closed",      "🔒 बंद"),        "#F9FAFB", "#475569", "#6B7280"),
        "rejected":    (t("❌ Rejected",    "❌ अस्वीकृत"),   "#FFF1F2", "#BE123C", "#EF4444"),
    }
    PRIORITY_CFG = {
    ...
    STATUS_OPTIONS = {
        t("All Statuses", "सभी स्थितियां"): "all",
        t("⏳ Pending",     "⏳ लंबित"):     "pending",
        t("🔄 In Progress", "🔄 प्रगति"):    "in_progress",
        t("✅ Resolved",    "✅ समाधान"):    "resolved",
        t("🔒 Closed",      "🔒 बंद"):       "closed",
        t("❌ Rejected",    "❌ अस्वीकृत"):  "rejected",
    }
    PRIORITY_OPTIONS = {
        t("All Priorities", "सभी प्राथमिकताएं"): "all",
        t("🔴 High",   "🔴 उच्च"):  "high",
        t("🟡 Medium", "🟡 मध्यम"): "medium",
        t("🟢 Low",    "🟢 निम्न"): "low",
    }
    ...
    def build_tl(status: str) -> str:
        ...
        out = "<div class='tl-wrap'><div style='display:flex;gap:0;'>"
        for i, label in enumerate(steps):
            ...
            out += (
                ...
                f"<div class='tl-dot {cls}' style='margin:0 auto 6px;'>{icon}</div>"
                f"<div style='font-size:.65rem;font-weight:600;"
                f"color:{'#6366F1' if cls in ('done','active') else '#9CA3AF'};'>"
                f"{label}</div></div>"
            )
            if i < len(steps) - 1:
                line_col = "#6366F1" if i < current else "#E5E7EB"
                out += (
                    f"<div style='flex:0 0 20px;display:flex;align-items:flex-start;"
                    f"padding-top:13px;'>"
                    f"<div style='height:2px;width:100%;background:{line_col};"
    ...
    def build_sla(comp: dict) -> str:
        ...
        if comp.get("is_overdue"):
            return (
                f"<div class='sla-late'>⏰&nbsp;<span>"
                f"<strong>{t('OVERDUE!','समय सीमा समाप्त!')}</strong>"
                f" {t('Expected by','अपेक्षित')}: {sla}</span></div>"
            )
        return (
            f"<div class='sla-ok'>⏱&nbsp;<span>"
            f"{t('Expected by','अपेक्षित')}: <strong>{sla}</strong></span></div>"
        )
    ...
    def render_card(comp: dict, expanded: bool = False, idx: int = 0,
        ...
        emg      = bool(comp.get("is_emergency", False))

        slbl, sbg, sfg, bord_col = (
            STATUS_CFG.get(status, (status.title(), "#F9FAFB", "#475569", "#6B7280"))[:4]
        )
        plbl, pbg, pfg = (
            PRIORITY_CFG.get(priority, (priority.title(), "#F9FAFB", "#475569"))[:3]
        )
        if emg:
            bord_col = "#DC2626"
        ...
        cat_icon = cat_icons.get(cat.lower(), "📋")

        exp_title = (
            f"{'🚨 ' if emg else ''}#{cid}  ·  {cat}  ·  {slbl}"
        )
        ...
        with st.expander(exp_title, expanded=expanded):
            ...
            emg_badge = (
                ...
                'style="background:#DC2626;color:#fff;margin-left:4px;">'
                '🚨 EMERGENCY</span>'
            ) if emg else ""

            st.markdown(
                f"<div class='trk-card' style='border-left-color:{bord_col};margin-top:8px;'>"

                # top row: ID + status + priority + emergency
                f"<div class='trk-card-top'>"
                f"<span class='trk-cid'>#{cid}</span>"
                f"<span class='trk-badge' style='background:{sbg};color:{sfg};'>{slbl}</span>"
                f"<span class='trk-badge' style='background:{pbg};color:{pfg};'>{plbl}</span>"
                f"{emg_badge}"
                f"</div>"

                # category + description
                f"<div class='trk-category'>{cat_icon} {html.escape(cat)}</div>"
                f"<div class='trk-desc'>"
                f"{html.escape(desc[:220])}{'…' if len(desc) > 220 else ''}"
                f"</div>"

                # meta
                f"<div class='trk-meta'>"
                f"<span>📍 {html.escape(str(loc))}</span>"
                f"<span>📅 {html.escape(str(date))}</span>"
                f"</div>"
                f"</div>",
            ...
            st.markdown(
                ...
                f"letter-spacing:.08em;color:#9CA3AF;margin:16px 0 8px;"
                f"display:flex;align-items:center;gap:8px;'>"
                f"<span style='width:3px;height:12px;background:linear-gradient("
                f"180deg,#6366F1,#8B5CF6);border-radius:3px;display:inline-block;"
                f"flex-shrink:0;'></span>"
                f"{t('Progress Timeline','प्रगति टाइमलाइन')}</div>"
                + build_tl(status)
                + build_sla(comp),
                unsafe_allow_html=True,
            )
    ...
    # ══════════════════════════════════════════════════════════════════════
    st.markdown(
        f"<div class='prem-hero' style='padding:28px 28px 24px;margin-bottom:0;'>"
        f"<div class='prem-hero-avatar'>🔍</div>"
        f"<div class='prem-hero-title'>{t('Track Complaint','शिकायत ट्रैक करें')}</div>"
        f"<p class='prem-hero-sub'>"
        f"{t('Check real-time status of your complaints','अपनी शिकायत की स्थिति जानें')}"
        f"</p>"
        f"<div class='prem-hero-stats'>"
        f"<div class='prem-hstat h-blue'><div class='prem-hstat-num'>🔎</div>"
        f"<div class='prem-hstat-lbl'>{t('Track by ID','ID से ट्रैक')}</div></div>"
        f"<div class='prem-hstat h-amber'><div class='prem-hstat-num'>📋</div>"
        f"<div class='prem-hstat-lbl'>{t('My Complaints','मेरी शिकायतें')}</div></div>"
        f"<div class='prem-hstat h-green'><div class='prem-hstat-num'>⏱</div>"
        f"<div class='prem-hstat-lbl'>{t('Live Status','लाइव स्थिति')}</div></div>"
        f"<div class='prem-hstat h-red'><div class='prem-hstat-num'>🚨</div>"
        f"<div class='prem-hstat-lbl'>{t('Emergency','आपातकाल')}</div></div>"
        f"</div></div>",
        unsafe_allow_html=True,
    )
    ...
    # ══════════════════════════════════════════════════════════════════════
    tab1, tab2 = st.tabs([
        f"🔎 {t('Track by ID',   'ID से ट्रैक करें')}",
        f"📋 {t('My Complaints', 'मेरी शिकायतें')}",
    ])
    ...
    with tab1:
        st.markdown(
            f"<div class='trk-section'>"
            f"🔎 {t('Search by Complaint ID','शिकायत ID से खोजें')}</div>",
            unsafe_allow_html=True,
        )
        ...
        with sc1:
            ...
            search_id = st.text_input(
                ...
                label_visibility="collapsed",
                placeholder=t(
                    "Enter complaint ID e.g. CMP-0041",
                    "शिकायत ID दर्ज करें जैसे CMP-0041",
                ),
                key="trk_search_id",
            )
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        if search_btn:
            ...
            if not sid:
                ...
                st.warning(t(
                    "Please login to track complaints.",
                    "शिकायतें ट्रैक करने के लिए लॉगिन करें।",
                ))
            else:
                with st.spinner(t("Searching…", "खोज रहे हैं…")):
                    raw = api("get", f"/complaints/user/{uid}")
                if isinstance(raw, list):
                    ...
                    if found:
                        ...
                        st.markdown(
                            f"<div class='trk-empty'>"
                            f"<span class='trk-empty-icon'>❌</span>"
                            f"<div class='trk-empty-title'>"
                            f"{t('Complaint not found','शिकायत नहीं मिली')}</div>"
                            f"<div class='trk-empty-sub'>"
                            f"{t('Check the ID and ensure you are logged into the correct account.','ID जाँचें या सुनिश्चित करें कि सही खाते में हैं।')}"
                            f"</div></div>",
                            unsafe_allow_html=True,
                        )
                else:
                    st.error(t(
                        ...
                        "डेटा प्राप्त नहीं हुआ। पुनः प्रयास करें।",
                    ))
    ...
    with tab2:
        if not uid:
            st.markdown(
                f"<div class='trk-empty'>"
                f"<span class='trk-empty-icon'>🔒</span>"
                f"<div class='trk-empty-title'>{t('Login Required','लॉगिन आवश्यक')}</div>"
                f"<div class='trk-empty-sub'>"
                f"{t('Please login to view your complaints.','अपनी शिकायतें देखने के लिए लॉगिन करें।')}"
                f"</div></div>",
                unsafe_allow_html=True,
            )
            return
        ...
        # ── fetch ────────────────────────────────────────────────────────
        with st.spinner(t("Loading complaints…", "शिकायतें लोड हो रही हैं…")):
            all_comps = api("get", f"/complaints/user/{uid}")
        ...
        if not all_comps:
            st.markdown(
                ...
                f"<span class='trk-empty-icon'>📭</span>"
                f"<div class='trk-empty-title'>"
                f"{t('No complaints yet','अभी कोई शिकायत नहीं')}</div>"
                f"<div class='trk-empty-sub'>"
                f"{t('File your first complaint to get started.','पहली शिकायत दर्ज करके शुरू करें।')}"
                f"</div></div>",
                unsafe_allow_html=True,
            )
        ...
        # ── session defaults ──────────────────────────────────────────────
        st.session_state.setdefault("trk_sf", list(STATUS_OPTIONS.values())[0])
        st.session_state.setdefault("trk_pf", list(PRIORITY_OPTIONS.values())[0])
        st.session_state.setdefault("trk_q",  "")

        # ── search bar ────────────────────────────────────────────────────
        st.markdown(
            f"<div class='trk-section'>"
            f"🔍 {t('Filter & Search','खोजें और फ़िल्टर करें')}</div>",
            unsafe_allow_html=True,
        )
        st.markdown('<div class="trk-search-wrap">', unsafe_allow_html=True)
        srch = st.text_input(
            ...
            label_visibility="collapsed",
            placeholder=t(
                "🔍  Search by ID, category, location…",
                "🔍  ID, श्रेणी, स्थान से खोजें…",
            ),
        ...
        st.markdown('<div class="trk-filter">', unsafe_allow_html=True)
        fd1, fd2 = st.columns(2)

        with fd1:
            status_label = t("Status", "स्थिति")
            # map current value back to its display label
            sf_current_label = next(
                (lbl for lbl, val in STATUS_OPTIONS.items()
                 if val == st.session_state.trk_sf),
                list(STATUS_OPTIONS.keys())[0],
            )
            sf_chosen_label = st.selectbox(
                status_label,
                options=list(STATUS_OPTIONS.keys()),
                index=list(STATUS_OPTIONS.keys()).index(sf_current_label),
                key="trk_sf_select",
            )
            new_sf = STATUS_OPTIONS[sf_chosen_label]
            if new_sf != st.session_state.trk_sf:
                st.session_state.trk_sf = new_sf
                st.rerun()

        with fd2:
            priority_label = t("Priority", "प्राथमिकता")
            pf_current_label = next(
                (lbl for lbl, val in PRIORITY_OPTIONS.items()
                 if val == st.session_state.trk_pf),
                list(PRIORITY_OPTIONS.keys())[0],
            )
            pf_chosen_label = st.selectbox(
                priority_label,
                options=list(PRIORITY_OPTIONS.keys()),
                index=list(PRIORITY_OPTIONS.keys()).index(pf_current_label),
                key="trk_pf_select",
            )
            new_pf = PRIORITY_OPTIONS[pf_chosen_label]
            if new_pf != st.session_state.trk_pf:
                st.session_state.trk_pf = new_pf
                st.rerun()
        ...
        if st.session_state.trk_sf != "all":
            active_tags.append(sf_chosen_label)
        if st.session_state.trk_pf != "all":
            active_tags.append(pf_chosen_label)
        if st.session_state.trk_q:
            active_tags.append(f'"{st.session_state.trk_q}"')

        if active_tags:
            tags_html = "".join(
                f"<span class='trk-filter-tag'>{tag}</span>"
                for tag in active_tags
            )
            st.markdown(
                f"<div class='trk-active-filters'>"
                f"<span style='font-size:.72rem;color:#9CA3AF;font-weight:600;'>"
                f"{t('Active:','सक्रिय:')}</span>{tags_html}</div>",
                unsafe_allow_html=True,
            )
        ...
        if st.session_state.trk_sf != "all":
            filtered = [c for c in filtered
                        if c.get("status","") == st.session_state.trk_sf]
        if st.session_state.trk_pf != "all":
            filtered = [c for c in filtered
                        if c.get("priority","") == st.session_state.trk_pf]
        if st.session_state.trk_q:
        ...
        # ── results summary row ───────────────────────────────────────────
        hidden = len(all_comps) - len(filtered)
        st.markdown(
            f"<div class='trk-count-row'>"
            f"<span class='trk-count-pill'>"
            f"{t('Showing','दिखा रहे हैं')} "
            f"<strong>{len(filtered)}</strong> "
            f"{t('of','में से')} "
            f"<strong>{len(all_comps)}</strong> "
            f"{t('complaints','शिकायतें')}"
            f"</span>"
            f"{'<span class=\"trk-count-hidden\">' + str(hidden) + ' ' + t('hidden','छिपे') + '</span>' if hidden else ''}"
            f"</div>",
            unsafe_allow_html=True,
        )
        ...
        if not filtered:
            st.markdown(
                ...
                f"<span class='trk-empty-icon'>🔍</span>"
                f"<div class='trk-empty-title'>"
                f"{t('No complaints match your filters','कोई शिकायत मेल नहीं खाती')}"
                f"</div>"
                f"<div class='trk-empty-sub'>"
                f"{t('Try adjusting the filters above.','ऊपर फ़िल्टर बदलकर देखें।')}"
                f"</div></div>",
                unsafe_allow_html=True,
            )
            return
        ...
        st.markdown(
            f"<div class='trk-section'>"
            f"📋 {t('Your Complaints','आपकी शिकायतें')}</div>",
            unsafe_allow_html=True,
        )
...
def pg_notifications():
    ...
    _A1   = "#6366F1"
    _A2   = "#8B5CF6"
    _UNR  =  "rgba(99,102,241,0.05)"
    
    # Red palette for delete (FIXED - no more black!)
    _RED       = "#EF4444"
    _RED_BG    =  "#FEF2F2"
    _RED_BG_H  =  "#FEE2E2"
    _RED_BD    =  "#FCA5A5"
...
.nc{{
    ...
    padding:18px 20px;margin:8px 0 0 0;position:relative;
    transition:transform .18s,box-shadow .18s,border-color .18s;
    font-family:'DM Sans','Noto Sans Devanagari',system-ui,sans-serif;
}}
.nc:hover{{transform:translateX(4px);box-shadow:0 8px 28px rgba(99,102,241,.15);border-color:{_A1}66;}}
.nc.unread{{border-left:4px solid {_A1};background:{_UNR};}}
.nc.dimmed{{opacity:.78;}}
...
.nc-icon{{font-size:1.75rem;min-width:44px;text-align:center;flex-shrink:0;margin-top:2px;
    background:linear-gradient(135deg,{_A1}22,{_A2}22);
    border-radius:12px;padding:6px;}}
.nc-body{{flex:1;min-width:0;}}
.nc-title{{font-size:.92rem;font-weight:700;color:{_TXT};margin-bottom:5px;
...
.nc-new{{background:linear-gradient(135deg,{_A1},{_A2});color:#fff;
    ...
    box-shadow:0 2px 8px rgba(99,102,241,.4);flex-shrink:0;}}
.nc-msg{{font-size:.83rem;color:{_SUB};line-height:1.65;margin-bottom:8px;
    word-wrap:break-word;}}
.nc-time{{font-size:.68rem;color:{_SUB};font-family:'DM Mono','Courier New',monospace;
    opacity:.85;}}
.nc-dot{{position:absolute;top:18px;right:18px;width:10px;height:10px;border-radius:50%;
    background:{_A1};box-shadow:0 0 0 4px rgba(99,102,241,.22);
    animation:ncPulse 2s ease-in-out infinite;}}
@keyframes ncPulse{{
    0%,100%{{box-shadow:0 0 0 4px rgba(99,102,241,.22);}}
    50%{{box-shadow:0 0 0 7px rgba(99,102,241,.10);}}
}}

/* ── section group label ── */
.ng-label{{font-size:.70rem;font-weight:800;text-transform:uppercase;letter-spacing:.12em;
    color:{_SUB};margin:24px 0 10px;display:flex;align-items:center;gap:10px;}}
.ng-label::before{{content:'';width:4px;height:14px;
...
.notif-stat:hover{{transform:translateY(-2px);box-shadow:0 8px 20px rgba(15,23,42,.10);}}
...
.notif-stat-num{{font-family:'Bricolage Grotesque','DM Sans',sans-serif;
    font-size:2.1rem;font-weight:800;line-height:1.1;margin-bottom:6px;}}
.notif-stat-lbl{{font-size:.66rem;font-weight:700;text-transform:uppercase;
    letter-spacing:.08em;color:{_SUB};}}

/* ════════════════════════════════════════════════════════════════════════
   FIX: Target buttons inside wrapper divs using :has() and direct selectors
   The wrapper div has class notif-read-btn / notif-del-btn
   ════════════════════════════════════════════════════════════════════════ */

/* READ button — blue tinted */
div.notif-read-btn-wrap div[data-testid="stButton"] > button,
div.notif-read-btn-wrap button[kind="secondary"],
div.notif-read-btn-wrap button{{
    background:{_BLU_BG} !important;
    color:{_BLU_TXT} !important;
    border:1.5px solid {_BLU_BD} !important;
    box-shadow:none !important;
...
div.notif-read-btn-wrap div[data-testid="stButton"] > button:hover,
div.notif-read-btn-wrap button:hover{{
    background:{_BLU_BG_H} !important;
    border-color:{_A1} !important;
    color:{_BLU_TXT} !important;
    transform:translateY(-1px) !important;
}}

/* DELETE button — red tinted (FIXED - no more black background!) */
div.notif-del-btn-wrap div[data-testid="stButton"] > button,
div.notif-del-btn-wrap button[kind="secondary"],
div.notif-del-btn-wrap button{{
    background:{_RED_BG} !important;
    color:{_RED_TXT} !important;
    border:1.5px solid {_RED_BD} !important;
    box-shadow:none !important;
    font-size:.75rem !important;
    font-weight:600 !important;
    padding:6px 12px !important;
...
div.notif-del-btn-wrap div[data-testid="stButton"] > button:hover,
div.notif-del-btn-wrap button:hover{{
    background:{_RED_BG_H} !important;
    border-color:{_RED} !important;
    color:{_RED_TXT} !important;
    transform:translateY(-1px) !important;
}}
div.notif-del-btn-wrap div[data-testid="stButton"] > button:focus,
div.notif-del-btn-wrap button:focus{{
    background:{_RED_BG_H} !important;
    color:{_RED_TXT} !important;
    border-color:{_RED} !important;
    box-shadow:0 0 0 3px {_RED_BG} !important;
}}
div.notif-read-btn-wrap div[data-testid="stButton"] > button:focus,
div.notif-read-btn-wrap button:focus{{
    background:{_BLU_BG_H} !important;
    color:{_BLU_TXT} !important;
    border-color:{_A1} !important;
    box-shadow:0 0 0 3px {_BLU_BG} !important;
}}
...
/* ── tip bar ── */
.notif-tip{{background:linear-gradient(135deg,{_CARD},{_CARD2});
    border:1px solid {_BOR};border-radius:14px;
    padding:14px 18px;display:flex;align-items:center;gap:12px;
    margin:18px 0;font-size:.78rem;color:{_SUB};line-height:1.6;
    box-shadow:0 2px 8px rgba(15,23,42,.04);}}
.notif-tip strong{{color:{_TXT};}}

/* ── empty state ── */
.notif-empty{{text-align:center;padding:3.5rem 2rem;background:{_CARD};
    border-radius:22px;border:1.5px dashed {_BOR};}}
.notif-empty-icon{{font-size:3.2rem;opacity:.5;display:block;margin-bottom:14px;}}
.notif-empty-title{{font-size:1rem;font-weight:700;color:{_TXT};margin-bottom:6px;}}
.notif-empty-sub{{font-size:.80rem;color:{_SUB};line-height:1.65;
    max-width:380px;margin:0 auto;}}

@media(max-width:600px){{
    .notif-stat-grid{{grid-template-columns:repeat(3,1fr);gap:8px;}}
    .notif-stat-num{{font-size:1.65rem;}}
    .notif-stat{{padding:14px 8px;}}
    .nc{{padding:14px 15px;border-radius:14px;}}
    .nc-icon{{font-size:1.4rem;min-width:38px;}}
    .nc-title{{font-size:.86rem;}}
    .nc-msg{{font-size:.78rem;}}
}}
</style>
""", unsafe_allow_html=True)
    ...
    def render_notif(n, show_read_btn=False, dimmed=False):
        ...
        # ── Action buttons row ───────────────────────────────────────────────
        st.markdown("<div class='notif-actions-row'>", unsafe_allow_html=True)

        if show_read_btn and is_unrd:
            btn_cols = st.columns([1.3, 1.3, 5], gap="small")

            # READ BUTTON — wrapped in styled div
            with btn_cols[0]:
                st.markdown("<div class='notif-read-btn-wrap'>", unsafe_allow_html=True)
                if st.button(
                    t("✓ Read","✓ पढ़ा"),
            ...
            # CLEAR BUTTON — wrapped in styled div
            with btn_cols[1]:
                st.markdown("<div class='notif-read-btn-wrap'>", unsafe_allow_html=True)
                if st.button(
                    t("🗑 Clear","🗑 हटाएं"),
            ...
            btn_cols = st.columns([1.3, 5.6], gap="small")
            with btn_cols[0]:
                st.markdown("<div class='notif-read-btn-wrap'>", unsafe_allow_html=True)
                if st.button(
                    t("🗑 Clear","🗑 हटाएं"),
...
_T = {
    "en": {
        ...
        "session_err": "⚠️ Session missing. Please login again.",
        "go_login": "Go to Login",
        "no_dept": "⚠️ No department assigned. Contact admin.",
        ...
        "unexpected": "Unexpected API response format",
        "load_err": "Could not load complaints",
        "total": "Total",
        "pending": "Pending",
        ...
        "search_ph": "ID, citizen, location…",
        "all": "All",
        "high": "High",
        "medium": "Medium",
        "low": "Low",
        "complaints_hdr": "📋 Complaints",
        "of": "of",
        "no_match": "No matching complaints",
        "adjust_filters": "Try adjusting your filters.",
        "no_complaints": "No complaints yet",
        "no_complaints_sub": "Complaints assigned to your department will appear here.",
        "image": "📷 Image",
        "evidence_hdr": "🖼️ Complaint Evidence",
        "view_photo": "👁️ View Uploaded Photo",
        "photo_caption": "Uploaded Complaint Evidence",
        "satisfied": "👍 Citizen satisfied with resolution",
        "not_satisfied": "⚠️ Citizen not satisfied – complaint reopened",
        "start": "🔄 Start Processing",
        "resolve_direct": "✅ Resolve Directly",
        "reject": "❌ Reject",
        "resolution_note": "📝 Resolution note",
        "res_note_ph": "Describe how issue was fixed…",
        "mark_resolved": "✅ Mark Resolved",
        "update_note": "📝 Update Note",
        "resolved_info": "✅ This complaint is resolved. It will be closed after citizen feedback.",
        "rejected_warn": "❌ This complaint was rejected.",
        "started": "Started!",
        ...
        "tip1": "Resolve complaints within 7 days for a better score.",
        "tip2": "Add detailed notes when resolving for citizen satisfaction.",
        "tip3": "Mark emergencies as high priority immediately.",
    },
    "hi": {
        "lang_label": "🌐 Language / भाषा",
        "session_err": "⚠️ सत्र अनुपलब्ध। कृपया पुनः लॉगिन करें।",
        "go_login": "लॉगिन पर जाएं",
        "no_dept": "⚠️ कोई विभाग नहीं दिया गया। व्यवस्थापक से संपर्क करें।",
        ...
        "unexpected": "अप्रत्याशित API प्रतिक्रिया प्रारूप",
        "load_err": "शिकायतें लोड नहीं हो सकीं",
        "total": "कुल",
        "pending": "लंबित",
        ...
        "search_ph": "ID, नागरिक, स्थान…",
        "all": "सभी",
        "high": "उच्च",
        "medium": "मध्यम",
        "low": "निम्न",
        "complaints_hdr": "📋 शिकायतें",
        "of": "में से",
        "no_match": "कोई मिलती शिकायत नहीं",
        "adjust_filters": "फ़िल्टर बदलने का प्रयास करें।",
        "no_complaints": "अभी कोई शिकायत नहीं",
        "no_complaints_sub": "आपके विभाग को दी गई शिकायतें यहाँ दिखेंगी।",
        "image": "📷 छवि",
        "evidence_hdr": "🖼️ शिकायत साक्ष्य",
        "view_photo": "👁️ अपलोड की गई फ़ोटो देखें",
        "photo_caption": "अपलोड किया गया शिकायत साक्ष्य",
        "satisfied": "👍 नागरिक समाधान से संतुष्ट है",
        "not_satisfied": "⚠️ नागरिक असंतुष्ट – शिकायत पुनः खोली गई",
        "start": "🔄 प्रक्रिया शुरू करें",
        "resolve_direct": "✅ सीधे हल करें",
        "reject": "❌ अस्वीकार करें",
        "resolution_note": "📝 समाधान नोट",
        "res_note_ph": "बताएं कि समस्या कैसे ठीक हुई…",
        "mark_resolved": "✅ हल चिह्नित करें",
        "update_note": "📝 नोट अपडेट करें",
        "resolved_info": "✅ यह शिकायत हल हो गई है। नागरिक प्रतिक्रिया के बाद बंद होगी।",
        "rejected_warn": "❌ यह शिकायत अस्वीकार कर दी गई थी।",
        "started": "शुरू किया!",
        "resolved_ok": "हल हो गया!",
        "rejected_ok": "अस्वीकृत",
        "note_updated": "नोट अपडेट हुआ",
        "active": "सक्रिय",
        "complaint": "शिकायत",
        "emergency": "आपातकाल",
        ...
        "filter_in_progress": "प्रक्रियाधीन",
        "filter_resolved": "हल",
        "filter_rejected": "अस्वीकृत",
        "tips_title": "💡 त्वरित सुझाव",
        "tip1": "बेहतर स्कोर के लिए 7 दिनों में शिकायत हल करें।",
        "tip2": "नागरिक संतुष्टि के लिए हल करते समय विस्तृत नोट जोड़ें।",
        "tip3": "आपातकालीन शिकायतों को तुरंत उच्च प्राथमिकता दें।",
    },
}
...
# ── CSS (inline — mirrors NagarSeva design tokens) ────────────────────────────
DASHBOARD_CSS = """
<style>
@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+Devanagari:wght@400;500;600;700&family=Inter:wght@300;400;500;600;700;800;900&family=Bricolage+Grotesque:opsz,wght@12..96,700;12..96,800&display=swap');
...
:root {
    ...
    --c-a2:      #D4500B;
    --c-a3:      #F07D3A;
    --c-a1-glow: rgba(232,89,12,0.18);
    --c-a1-soft: rgba(232,89,12,0.08);
    --sh-sm:     0 1px 3px rgba(26,20,10,.06),0 4px 12px rgba(26,20,10,.05);
    --sh-md:     0 4px 16px rgba(26,20,10,.08),0 12px 32px rgba(26,20,10,.06);
    --sh-lg:     0 12px 48px rgba(26,20,10,.10),0 32px 64px rgba(26,20,10,.07);
    --r-sm: 10px; --r-md: 14px; --r-lg: 18px; --r-xl: 22px; --r-2xl: 28px;
    --t-fast: 0.15s cubic-bezier(.4,0,.2,1);
    --t-base: 0.22s cubic-bezier(.4,0,.2,1);
    /* semantic */
    --green-bg:#F0FDF4; --green-bd:#86EFAC; --green-tx:#166534;
    --amber-bg:#FFF7ED; --amber-bd:#FED7AA; --amber-tx:#C2410C;
    --red-bg:  #FEF2F2; --red-bd:  #FECACA; --red-tx:  #991B1B;
    --blue-bg: #EFF6FF; --blue-bd: #BFDBFE; --blue-tx: #1E40AF;
}
...
/* ── HERO ── */
.off-hero {
    background: linear-gradient(135deg, #2D3A1F 0%, #1E2D14 100%);
...
.off-hero::before {
    ...
    width:55%; height:160%;
    background: radial-gradient(circle,rgba(232,89,12,.18) 0%,transparent 70%);
...
.off-hero-dept {
    ...
    font-size:.75rem; color:#FED7AA; margin-top:6px;
}
.off-hero-email { font-size:.75rem; color:#CBD5E1; margin-top:3px; }
.off-hero-badge {
...
.off-prog-fill {
    height:100%;
    background:linear-gradient(90deg,var(--c-a1),#10B981);
    border-radius:20px;
    transition:width .5s cubic-bezier(.4,0,.2,1);
}
.off-prog-meta {
    display:flex; justify-content:space-between;
    margin-top:.5rem; font-size:.68rem; color:var(--c-sub);
}
...
.off-comp-card.pri-high   { border-left:4px solid var(--red-tx); }
.off-comp-card.pri-medium { border-left:4px solid var(--amber-tx); }
.off-comp-card.pri-low    { border-left:4px solid var(--green-tx); }
 
.off-comp-body { padding:1rem 1.1rem; }
...
.off-comp-title {
    font-weight:800; font-size:.9rem; color:var(--c-text);
    letter-spacing:-.02em;
}
.off-comp-badges { display:flex; gap:.5rem; flex-wrap:wrap; align-items:center; }
...
.badge-pending    { background:var(--amber-bg); color:var(--amber-tx); border:1px solid var(--amber-bd); }
.badge-in_progress{ background:var(--blue-bg);  color:var(--blue-tx);  border:1px solid var(--blue-bd); }
.badge-resolved   { background:var(--green-bg); color:var(--green-tx); border:1px solid var(--green-bd); }
.badge-closed     { background:#F3F4F6; color:#374151; border:1px solid #D1D5DB; }
.badge-rejected   { background:var(--red-bg);   color:var(--red-tx);   border:1px solid var(--red-bd); }
.badge-pri-high   { background:var(--red-bg);   color:var(--red-tx);   border:1px solid var(--red-bd); }
.badge-pri-medium { background:var(--amber-bg); color:var(--amber-tx); border:1px solid var(--amber-bd); }
.badge-pri-low    { background:var(--green-bg); color:var(--green-tx); border:1px solid var(--green-bd); }
.badge-img        { background:#EEF2FF; color:#6366F1; border:1px solid #C7D2FE; }
.badge-emerg      { background:#FEF2F2; color:#991B1B; border:1px solid #FECACA;
...
.off-meta {
    ...
    font-size:.75rem; color:var(--c-sub); margin-bottom:.75rem;
}
...
.off-empty-icon { font-size:3rem; display:block; margin-bottom:.75rem; }
...
.off-section-title { font-weight:700; font-size:.95rem; color:var(--c-text); }
.off-section-badge {
    background:var(--c-card2); border:1px solid var(--c-border);
    padding:2px 12px; border-radius:30px;
    font-size:.7rem; color:var(--c-sub); font-weight:600;
}
 
/* ── TIPS CARD ── */
.off-tips {
    background:linear-gradient(135deg,#F0F9FF,#ECFDF5);
    border:1.5px solid #CCF0E6;
    border-radius:var(--r-xl);
    padding:1rem 1.25rem;
    margin-top:1.5rem;
}
.off-tips-title { font-weight:800; margin-bottom:.5rem; font-size:.85rem; }
.off-tips li { font-size:.78rem; color:var(--c-sub); margin:.3rem 0; }
 
/* ── EXPANDER OVERRIDE ── */
details {
    background:var(--c-card) !important;
    border:1.5px solid #FDEAD7 !important;
    border-radius:var(--r-lg) !important;
    margin-bottom:6px !important;
    overflow:hidden !important;
}
details[open] { border-color:var(--c-a1) !important; box-shadow:0 4px 16px rgba(232,89,12,.10) !important; }
details summary {
    background:#FFF7ED !important; color:var(--c-text) !important;
    font-weight:700 !important; font-size:.82rem !important;
    padding:11px 15px !important; border-radius:var(--r-lg) !important;
    border:none !important; cursor:pointer !important;
    transition:all var(--t-fast) !important;
}
details[open] summary { border-radius:var(--r-lg) var(--r-lg) 0 0 !important; background:#FFF1E0 !important; border-bottom:1px solid #FDEAD7 !important; }
details summary:hover { background:#FFEDD5 !important; color:#EA580C !important; }
details > div { background:#FFFBF5 !important; padding:14px 16px !important; }
...
.stButton > button:hover { transform: translateY(-2px) !important; box-shadow: var(--sh-sm) !important; }
...
/* ── RESPONSIVE ── */
@media(max-width:768px) {
    .off-stat-grid { grid-template-columns:repeat(2,1fr); gap:.75rem; }
    .off-hero { padding:1.25rem; border-radius:var(--r-xl); }
    .off-hero-avatar { width:50px; height:50px; font-size:1.4rem; }
    .off-comp-hdr { flex-direction:column; align-items:flex-start; }
}
@media(max-width:480px) {
    .off-stat-grid { grid-template-columns:repeat(2,1fr); }
    .off-stat-num { font-size:1.5rem; }
    .off-hero-name { font-size:1.1rem; }
}
</style>
"""
...
def _status_badge(status: str) -> str:
    mapping = {
        ...
        "in_progress": ("status_in_progress", "badge-in_progress"),
        "resolved":    ("status_resolved",    "badge-resolved"),
        "closed":      ("status_closed",      "badge-closed"),
        "rejected":    ("status_rejected",    "badge-rejected"),
    }
    key, cls = mapping.get(status, ("status_pending", "badge-pending"))
    return _badge(_t(key), cls)
 
 
def _priority_badge(priority: str) -> str:
    mapping = {
        "high":   ("pri_high",   "badge-pri-high"),
        "medium": ("pri_medium", "badge-pri-medium"),
        "low":    ("pri_low",    "badge-pri-low"),
    }
    key, cls = mapping.get(priority, ("pri_normal", "badge-pri-medium"))
    return _badge(_t(key), cls)
...
def pg_official_dashboard():
    ...
    * {
    ...
    /* ── Expander fix — light orange theme (both old + new Streamlit) ── */
    /* New Streamlit (details/summary) */
    details {
        background: #FFFFFF !important;
        border: 1.5px solid #FDEAD7 !important;
    ...
    /* Old Streamlit classes (fallback) */
    ...
    /* Expander text/paragraph color fix */
...
""", unsafe_allow_html=True)
    ...
    if approved:
        ...
        for o in approved:
            ...
            with rc2:
                st.markdown("<div class='ooff-stat-pill-num'>", unsafe_allow_html=True)
                if st.button("🗑️", key=f"rem_{oid}", use_container_width=True,):
                    api("put", f"/admin/officials/{oid}/reject")
                    st.rerun()
                st.markdown("</div>", unsafe_allow_html=True)
