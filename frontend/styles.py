...
/* FIX 1: Dropdown popover background & text */
...
/* FIX 2: Scope the wildcard reset to non-option children only to prevent text wipeout */
...
/* FIX 6: Radio labels — visible in dark mode */
...
/* FIX 7: Notification bar text — prevent global div reset from overriding */
.prem-notif-bar-text{{color:{amber_text}!important;}}
.prem-notif-bar-badge{{color:#fff!important;background:{amber_text}!important;}}

/* FIX 8: Complaint description box text */
.complaint-desc-box{{color:{text}!important;}}

/* FIX 9: prem-badge-closed — card_bg2 is too dark in dark mode, use hover_bg for contrast */
...
.stButton>button:hover{{
    ...
    background:linear-gradient(135deg,{accent2},{accent3})!important;
}}
...
.prem-empty-state,
.prem-empty-state div,
.prem-empty-state span,
.prem-empty-state p{{
    color: var(--c-text) !important;
    opacity: 1 !important;
}}
...
/* dark mode support */
[data-theme="dark"] .prem-empty-state,
[data-theme="dark"] .prem-empty-state *{{
    color: #EFF2FF !important;
}}
.TextInput>div>div>input:focus,
.stTextArea>div>div>textarea:focus{{
    border-color:var(--c-a1)!important;
    box-shadow:0 0 0 3px var(--c-a1-glow)!important;
    outline:none!important;
    background:var(--c-card)!important;
}}
...
/* radio label — base (light mode) */
.stRadio label{{text-transform:none!important;letter-spacing:0!important;font-size:var(--fs-base)!important;color:var(--c-text)!important;}}
...
.stTabs [data-baseweb="tab-list"]{{
    ...
    padding:5px!important;border:1px solid var(--c-border)!important;
...
/* FIX: hover_bg has better contrast than card_bg2 in dark mode */
.prem-badge-closed  {{background:{hover_bg};color:{subtext};   border:1px solid {border};  }}
...
.prem-notif-card{{
    ...
    margin:var(--sp-2) 0;border:1px solid var(--c-border);
...
/* FIX: added !important — global div reset was overriding this */
.prem-notif-bar-text{{font-size:var(--fs-sm);color:{amber_text}!important;font-weight:600;flex:1;}}
.prem-notif-bar-badge{{background:{amber_text};color:#fff!important;border-radius:99px;padding:2px 9px;font-size:var(--fs-2xs);font-weight:700;}}
...
.prem-empty-state{{
    ...
    border:1px dashed var(--c-border);position:relative;overflow:hidden;
}}
...
.prem-empty-icon {{font-size:var(--icon-2xl);margin-bottom:var(--sp-4);opacity:.85;display:block;position:relative;z-index:1;}}
...
/* FIX STREAMLIT SECONDARY BUTTONS */
...
.prem-filter-chip{{
    ...
    font-size:var(--fs-xs);font-weight:700;
    border:1px solid var(--c-border);background:var(--c-card);color:var(--c-sub);
...
/* FIX: Force text color on complaint description box in dark mode */
.complaint-desc-box{{
    background:var(--c-card2);padding:var(--sp-3) var(--sp-4);
    border-radius:var(--r-md);margin:var(--sp-3) 0;
    border:1px solid var(--c-border);color:var(--c-text)!important;
