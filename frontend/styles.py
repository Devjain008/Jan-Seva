def get_css(dark_mode=False):
    # ── colour tokens ─────────────────────────────────────────────────────────
    # Light theme matches NagarSeva screenshot:
    #   bg = warm white/cream, accent = warm orange, header = dark olive
    # Dark theme = deep navy variant (unchanged from original intent)
    if dark_mode:
        bg          = "#0E1015"
        card_bg     = "#161B24"
        card_bg2    = "#1C2333"
        ...
        accent2     = "#F07D3A"
        accent3     = "#D4500B"
        a1_glow     = "rgba(232,89,12,0.30)"
        a1_soft     = "rgba(232,89,12,0.10)"
        a2_soft     = "rgba(240,125,58,0.08)"
        hero_from   = "#2D3A1F"
        hero_mid    = "#3D4F2A"
        hero_to     = "#1A2410"
        ...
        # ── LIGHT THEME — NagarSeva / warm orange palette ──────────────────
        bg          = "#FAF8F5"          # warm cream background
        card_bg     = "#FFFFFF"          # pure white cards
        card_bg2    = "#FDF9F6"          # very warm off-white
        text        = "#1A1A1A"          # near-black text
        subtext     = "#6B7280"          # neutral mid-gray subtext
        border      = "#EDE8E2"          # warm light border
        input_bg    = "#F5F2EE"          # warm input background
        sidebar_bg  = "#FFFFFF"
        hover_bg    = "#F5F0EA"          # warm hover
        tag_bg      = "#FFF0E6"          # light orange tint tag
        ...
        hero_from   = "#2D3A1F"          # dark olive (exact header color)
        hero_mid    = "#3D5028"
        hero_to     = "#1E2D14"
...
/* ── TOP STRIPE — warm orange ── */
.stApp::before{{
    content:'';display:block;position:fixed;top:0;left:0;right:0;height:3px;
...
header[data-testid="stHeader"]{{
    ...
    overflow:visible!important;z-index:99998!important;
}}
button[kind="header"]{{
...
.stTabs [data-baseweb="tab"]{{
    ...
    color:{subtext}!important;padding:8px 18px!important;
...
.notif-del-btn .stButton>button{{
    ...
    color:#DC2626!important;border:1.5px solid rgba(220,38,38,.18)!important;
    box-shadow:0 4px 12px rgba(15,23,42,.05)!important;filter:none!important;
}}
.notif-del-btn .stButton>button:hover{{
    background:#FEF2F2!important;background-image:none!important;
    color:#B91C1C!important;border:1.5px solid rgba(220,38,38,.24)!important;
...
/* ── HERO BANNER — dark olive like the screenshot ── */
.prem-hero{{
...
.prem-action-card{{
    ...
    box-shadow:{shadow_sm};margin-bottom:var(--sp-1);
    position:relative;overflow:hidden;
}}
...
.prem-action-label{{
    ...
    color:{text};line-height:1.35;letter-spacing:-0.01em;
}}
...
.prem-complaint-item:hover{{
    ...
    border-left-color:{accent2};box-shadow:{shadow_md};
}}
.prem-complaint-id{{
...
.prem-complaint-title{{
    ...
    color:{text};line-height:1.4;letter-spacing:-0.01em;
}}
.prem-complaint-desc{{
    font-size:var(--fs-base);color:{subtext};line-height:1.65;margin:var(--sp-1) 0 var(--sp-3);
}}
.prem-complaint-meta{{
    font-size:var(--fs-xs);color:{subtext};
...
.prem-badge{{
    ...
    font-size:var(--fs-2xs);font-weight:700;display:inline-block;
...
.prem-tip-bar{{
    ...
    margin-top:var(--sp-6);box-shadow:{shadow_sm};
...
.prem-divider{{
    display:flex;align-items:center;gap:var(--sp-3);
    margin:var(--sp-5) 0;color:{subtext};
...
.prem-filter-chip{{
    ...
    font-size:var(--fs-xs);font-weight:700;
    border:1.5px solid {border};background:{card_bg};color:{subtext};
...
.prem-form-section-label{{
    font-size:var(--fs-xs);font-weight:700;text-transform:uppercase;
    letter-spacing:0.09em;color:{subtext};margin-bottom:var(--sp-3);
    display:flex;align-items:center;gap:7px;
}}
.prem-form-section-label::before{{
    content:'';width:3px;height:13px;
    background:linear-gradient(180deg,{accent1},{accent3});border-radius:99px;
}}
...
.complaint-desc-box{{
    background:{card_bg2};padding:var(--sp-3) var(--sp-4);
...
.cc-meta-item{{color:#374151!important;}}
.cc-description,.cc-desc{{color:#374151!important;}}
...
.hero p{{
    font-size:var(--fs-base);margin:0;opacity:0.85;position:relative;z-index:1;
    color:rgba(255,255,255,0.88);font-weight:400;
}}
.card{{
    background:{card_bg};border-radius:var(--r-lg);
    ...
    border:1.5px solid {border};box-shadow:{shadow_sm};
    transition:transform var(--t-base),box-shadow var(--t-base),border-color var(--t-fast);
    position:relative;overflow:hidden;animation:prem-fade-up 0.28s ease both;
}}
...
.stat-card{{
    ...
    position:relative;overflow:hidden;
    transition:transform var(--t-base),box-shadow var(--t-base);
    animation:prem-scale-in 0.28s ease both;
}}
.stat-card::before{{
    content:'';position:absolute;top:0;left:0;right:0;height:3px;
    background:linear-gradient(90deg,{accent1},{accent3});
...
.action-icon{{
    ...
    margin:0 auto var(--sp-3);background:{a1_soft};border:1.5px solid {accent1}22;
    transition:transform var(--t-fast);
}}
...
.complaint-item{{
    ...
    box-shadow:{shadow_sm};
    transition:transform var(--t-fast),border-color var(--t-fast),box-shadow var(--t-base);
    animation:prem-fade-up 0.28s ease both;
}}
...
.welcome-title{{
    font-family:'Bricolage Grotesque','Inter',sans-serif;
    font-size:var(--fs-3xl);font-weight:800;margin-bottom:var(--sp-3);
    color:{text};letter-spacing:-0.04em;
    background:linear-gradient(135deg,{accent1},{accent2});
...
div[data-testid="stMetric"] label{{
    ...
    font-size:var(--fs-2xs)!important;font-weight:700!important;color:{subtext}!important;
}}
div[data-testid="stMetric"] [data-testid="stMetricValue"]{{
    ...
    letter-spacing:-0.03em!important;color:{text}!important;
}}
...
.dd-remove-wrap .stButton > button{{
    width:54px!important;min-width:54px!important;height:46px!important;
    border-radius:14px!important;border:none!important;
    background:linear-gradient(135deg,#EF4444,#DC2626)!important;
    color:#FFFFFF!important;font-size:18px!important;font-weight:700!important;
    box-shadow:0 10px 24px rgba(239,68,68,.24)!important;
    transition:all .18s ease!important;
}}
...
@media(max-width:768px){{
    ...
    .stat-card{{border-radius:var(--r-md);padding:var(--sp-4) var(--sp-3) var(--sp-3);}}
    .stat-number{{font-size:var(--fs-2xl);}}
    .action-card{{padding:var(--sp-5) var(--sp-3) var(--sp-4);border-radius:var(--r-md);}}
    .action-icon{{width:46px;height:46px;font-size:var(--icon-lg);}}
    .action-label{{font-size:var(--fs-xs);}}
    .complaint-item,.notif-card,.card{{border-radius:var(--r-md);}}
    .welcome-title{{font-size:var(--fs-2xl);}}
    .welcome-screen{{padding:40px 16px 24px;}}
}}
@media(max-width:480px){{
    ...
    .stat-card{{padding:var(--sp-3) var(--sp-2) var(--sp-2);}}
