...
from backend.models import Official   # adjust model name if different

router = APIRouter(prefix="/auth", tags=["auth"])
pwd_context = CryptContext(schemes=["bcrypt"], deprecated="auto")
# add at the top with other models
class PasswordResetRequest(BaseModel):
    email: EmailStr
    new_password: str

@router.post("/official/reset-password")
async def official_reset_password(req: PasswordResetRequest, db: Session = Depends(get_db)):
    try:
        ...
        if not official:
            return {"success": False, "detail": "No account found with this email"}
        
        # Hash new password
        hashed = pwd_context.hash(req.new_password)
        
        # Update password_hash column (as per your model)
        official.password_hash = hashed
        db.commit()
        
        return {"success": True, "message": "Password updated successfully"}
    
    except Exception as e:
        # Log the error to backend terminal
